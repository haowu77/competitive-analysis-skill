...
python3 scripts/build_benchmark_xlsx.py \
  --output examples/benchmark-auto.xlsx \
  --brief "AI meal planning for busy families"
```
...
## Input / Output
Input source:
- `--brief` (natural language requirement), or
- `--input-json` (structured benchmark payload)

Output is always 5 sheets in fixed order:
1. `Summary`
2. `Benchmark`
3. `Feature-Matrix`
4. `Pricing-GTM`
...
## Quality & Trust
- Recommended >= 3 sources per competitor
- Recommend both official and third-party evidence
- Localization: `en/zh/ja/ko/es/fr/de`
- Regression tests: `python3 -m unittest discover -s tests -p 'test_*.py'`
- CI: GitHub Actions
...
## Use Cases
- early-stage market scan for product definition
- roadmap parity/gap decision support
- investor/review deck benchmark appendix
